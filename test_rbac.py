...
def test_public_post_dish_unauthorized(client):
    dish = {
        'name' : 'sushi', 'price' : 15,
...
def test_manager_post_dish_unauthorized(client, manager_header):
    dish = {
        'name' : 'sushi', 'price' : 15,
        'ingredient' : [{'name' : 'salmon', 'allergen' : 'seafood'},
                        {'name' : 'rice', 'allergen' : ''}]
    }
    res = client.post('/dish', json=dish, headers=manager_header)
    assert res.status_code == 401
...
def test_chef_post_dish(client, chef_header):
    dish = {
        'name' : 'sushi', 'price' : 15,
        'ingredient' : [{'name' : 'salmon', 'allergen' : 'seafood'},
                        {'name' : 'rice', 'allergen' : ''}]
    }
    res = client.post('/dish', json=dish, headers=chef_header)
    assert res.status_code == 200